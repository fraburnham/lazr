...
(defn ->gcode
  ([travel-speed laser-intensity pixels-per-mm image]
   (->gcode travel-speed laser-intensity pixels-per-mm image identity))
  ([travel-speed laser-intensity pixels-per-mm image transformer]
   (let [height (.getHeight image)
         width (.getWidth image)
         x-transformer (fn [x] (first (transformer [x 0])))
         y-transformer (fn [y] (second (transformer [0 y])))
