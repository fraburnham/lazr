...
(defrecord path->gcode [paths state]
  PathHandler
  ...
  (closePath [_]
    ...
    (set-positioning! paths state ::absolute)
    (swap! paths conj (merge {:type :g1} (::initial-point @state))))

  (curvetoCubicAbs [_ x1 y1 x2 y2 x y]
    (set-positioning! paths state ::absolute)
    (swap! paths conj {:type :g5 :i x1 :j y1 :p x2 :q y2 :x x :y y}))
  ...
  (linetoAbs [_ x y]
    (set-positioning! paths state ::absolute)
    (swap! paths conj {:type :g1 :x x :y y}))

  (linetoHorizontalAbs [_ x]
    (set-positioning! paths state ::absolute)
    (swap! paths conj {:type :g1 :x x}))
  ...
  (movetoAbs [_ x y]
    (println x y)
    (set-positioning! paths state ::absolute)
    (swap! paths conj {:type :g0 :x x :y y})
